...
| ContainsIn      | `(?:contains(?:Key?)(?:In?)\|isIn)<X>`<br>`(?:contains(?:In?)\|isIn)<X>`<br>`(?:contains\|isIn)<X>`                                                                 | `boolen containsKeyInApis(String key)`<br>`boolean isInApis(String... keys)`<br>`boolean isInIds(UUID uuid)`<br>`boolean containsIds(UUID... uuid)`<br>              | required: boolean type                                           |
| NotContainsIn   | `(?:(notContains\|excludes)(?:Key?)(?:In?)\|isNotIn)<X>`<br>`(?:(notContains\|excludes)(?:In?)\|isNotIn)<X>`<br>`(?:notContains\|excludes\|isNotIn)<X>` | `boolen excludesKeyInApis(String key)`<br>`boolean excludes(String... keys)`<br>`boolean isNotInIds(UUID uuid)`<br>`boolean notContainsIds(UUID... uuid)`<br>        | required: boolean type                                           |
